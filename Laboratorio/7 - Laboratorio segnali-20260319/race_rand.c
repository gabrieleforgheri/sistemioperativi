...
int main(void) {
    srand(getpid());  //no real randomness!
    pid_t children[NCHILD];

    printf("\033[2J");      /* clear screen */
    printf("\033a[?25l");    /* hide cursor */
    fflush(stdout);
    ...
    while (still_running > 0) {
    	    ...
    	    //now randomly wake up a horse that is still running
	while(1){
	    int lucky_horse = random()%NCHILD;
	    if (children[lucky_horse] == -1)
                continue;

            kill(children[lucky_horse], SIGCONT);
            usleep(SLICE_USEC);
            kill(children[lucky_horse], SIGSTOP);
	    break;
	}
