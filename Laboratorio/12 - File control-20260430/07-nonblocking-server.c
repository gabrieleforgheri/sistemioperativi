 ...
 *
 * Server behavior
 * - The server uses non-blocking reads, hence it loops through all fifos
 *   and try to read some data. If there is a request it manages the request.
 *   If there is nothing to read it goes through the next fifo. This
 *   keeps the process busy looping through fifos even if there is no 
 *   real work to do... 
 */  
...
int main(void) {
    Message msg;
    int request_fd[NCLIENTS]; //array of pollfd structs that we will use to poll()
	...
	request_fd[i] = open(fifo_name, O_RDWR | O_NONBLOCK);
        if (request_fd[i] < 0) {
    ...
    while (1) {
        ...
        for (int i = 0; i < NCLIENTS; i++) {
            ...
            ssize_t n = read(request_fd[i], &msg, sizeof(msg));
	    if (n == -1 && (errno == EAGAIN || errno == EWOULDBLOCK))
		    continue; //nothing to read from this fifo now... try the next one
