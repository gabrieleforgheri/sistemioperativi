 ...
 *
 * Server behavior
 * - the server uses normal, blocking read. There only thing we can do is block on
 *   read from the first client, then go through the next, and so on... 
 */  
...
int main(void) {
	...
	request_fd[i] = open(fifo_name, O_RDWR);
        if (request_fd[i] < 0) {
